...
class Game:
    def __init__(self, master, player1, player2, Q_learn=None, Q={}, alpha=0.3, gamma=0.9):
        ...
        for i in range(3):
            for j in range(3):
                self.buttons[i][j] = tk.Button(frame, height=4, width=12, text=self.empty_text, command=lambda i=i, j=j: self.callback(self.buttons[i][j]))
                self.buttons[i][j].grid(row=i, column=j)

        self.reset_button = tk.Button(height=1, width=24, text="Reset Game", fg='white', bg='teal',
                                      font=('Tahoma', 8, 'bold'), command=self.reset)
        self.reset_button.grid(row=3, padx=1, pady=3)

        self.new_game_button = tk.Button(height=1, width=24, text="New Game", fg='white', bg='navy',
                                         font=('Tahoma', 8, 'bold'), command=self.new_game)
        self.new_game_button.grid(row=4, padx=1, pady=3)

        self.exit_button = tk.Button(height=1, width=12, text="Close Game", fg='white', bg='black',
                                         font=('Tahoma', 8, 'bold'), command=lambda: sys.exit())
        self.exit_button.grid(row=5, padx=1, pady=3)
    ...
    def handle_move(self, move):
        ...
        if self.current_player.mark == "X":
            self.buttons[i][j].configure(text=self.current_player.mark, fg="blue") #lable red for X and label blue for O
        if self.current_player.mark == "O":
            self.buttons[i][j].configure(text=self.current_player.mark, fg="red")
    ...
    def declare_outcome(self):
        if self.board.winner() is None:
            ...
            print(("Player {mark} won!".format(mark=self.current_player.mark)))
...
class Board:
    ...
    def over(self):             # The game is over if there is a winner or if no squares remain empty (draw game)
    ...
    def give_reward(self):                          # Assign a reward for the player with mark X in the current board position.
        if self.over():
            if self.winner() is not None:
                ...
                return 0.5                          # A smaller positive reward for draw game
        else:
...
class RandomPlayer(ComputerPlayer):
    ...
    def get_move(board):
        ...
        if moves:   # If "moves" is not an empty list (as it would be if draw game were reached)
